...
async function textmakerCommand(dave, chatId, message, q, type) {
    try {
        if (!q) {
            return await dave.sendMessage(chatId, messageTemplates.error("Please provide text to generate\nExample: .metallic Nick"));
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await dave.sendMessage(chatId, messageTemplates.error("Invalid text generator type"));
            }
            ...
            await dave.sendMessage(chatId, messageTemplates.success(text, result.image));
        } catch (error) {
            console.error('Error in text generator:', error);
            await dave.sendMessage(chatId, messageTemplates.error(`Error: ${error.message}`));
        }
    } catch (error) {
        console.error('Error in textmaker command:', error);
        await dave.sendMessage(chatId, messageTemplates.error("An error occurred. Please try again later."));
    }
}
