...
async function songCommand(dave, chatId, message) {
    try {
        const text = message.message?.conversation || message.message?.extendedTextMessage?.text;
        const searchQuery = text.split(' ').slice(1).join(' ').trim();
        
        if (!searchQuery) {
            return await dave.sendMessage(chatId, { text: "What song do you want to download?" });
        }
        ...
        if (searchQuery.startsWith('http://') || searchQuery.startsWith('https://')) {
            ...
            if (!videos || videos.length === 0) {
                return await dave.sendMessage(chatId, { text: "No songs found!" });
            }
        ...
        try {
            ...
            return await dave.sendMessage(chatId, { text: "Failed to fetch download link. Try again later." });
                }
            }
        }
        ...
        if (!result || !result.status || !result.result || !result.result.download) {
            ...
            return await dave.sendMessage(chatId, { text: "Failed to get a valid download link from the API." });
        }
        ...
        try {
            ...
            return await dave.sendMessage(chatId, { text: "Failed to download the song (network error)." });
        }
        ...
        if (response.status < 200 || response.status >= 300) {
            ...
            return await dave.sendMessage(chatId, { text: "Failed to download the song file from the server (bad status)." });
        }
        ...
        if (!fileSize || fileSize < 10240) { // <10KB indicates failure
            return await dave.sendMessage(chatId, { text: "Song file seems invalid (too small). Please try again." });
        }
        ...
        await dave.sendMessage(chatId, { text: "Download failed. Please try again later." });
    }
}
