...
async function showTyping(dave, chatId) {
    try {
...
async function handleChatbotCommand(dave, chatId, message, match) {
    if (!match) {
        await showTyping(dave, chatId);
...
async function handleChatbotResponse(dave, chatId, message, userMessage, senderId) {
    const data = loadUserGroupData();
