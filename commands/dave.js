...
async function getQuotedOrOwnImageUrl(dave, message) {
    ...
    try {
        const url = await dave.profilePictureUrl(targetJid, 'image');
        return url;
    } catch {
...
async function handleHeart(dave, chatId, message) {
    try {
        const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
        const url = `https://api.some-random-api.com/canvas/misc/heart?avatar=${encodeURIComponent(avatarUrl)}`;
        const response = await axios.get(url, { responseType: 'arraybuffer' });
        await dave.sendMessage(chatId, { image: Buffer.from(response.data) }, { quoted: message });
    } catch (error) {
        console.error('Error in misc heart:', error);
        await dave.sendMessage(chatId, { text: '❌ Failed to create heart image. Try again later.' }, { quoted: message });
    }
}

async function miscCommand(dave, chatId, message, args) {
    ...
    try {
        switch (sub) {
            ...
            case 'its-so-stupid': {
                ...
                if (!dog) {
                    await dave.sendMessage(chatId, { text: 'Usage: .misc its-so-stupid <text>' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
            ...
            case 'namecard': {
                ...
                if (!username || !birthday) {
                    await dave.sendMessage(chatId, { text: 'Usage: .misc namecard username|birthday|description(optional)' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
            ...
            case 'oogway2': {
                ...
                if (!quote) {
                    await dave.sendMessage(chatId, { text: `Usage: .misc ${sub} <quote>` }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
            ...
            case 'tweet': {
                ...
                if (!displayname || !username || !comment) {
                    await dave.sendMessage(chatId, { text: 'Usage: .misc tweet displayname|username|comment|theme(optional light/dark)' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
            ...
            case 'youtube-comment': {
                ...
                if (!username || !comment) {
                    await dave.sendMessage(chatId, { text: 'Usage: .misc youtube-comment username|comment' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(dave, message);
            ...
            default:
                await dave.sendMessage(chatId, { text: 'Usage: .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <text>|namecard u|b|d?|nobitches <text>|oogway <q>|oogway2 <q>|tweet dn|un|c|theme?|youtube-comment un|c>' }, { quoted: message });
                break;
        }
    } catch (error) {
        console.error('Error in misc command:', error);
        await dave.sendMessage(chatId, { text: '❌ Failed to generate image. Check your parameters and try again.' }, { quoted: message });
    }
}
