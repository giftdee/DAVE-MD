...
async function demoteCommand(dave, chatId, mentionedJids, message) {
    try {
        ...
        try {
            const adminStatus = await isAdmin(dave, chatId, message.key.participant || message.key.remoteJid);
        ...
        await dave.groupParticipantsUpdate(chatId, userToDemote, "demote");
...
async function handleDemotionEvent(dave, groupId, participants, author) {
    try {
