...
async function attpCommand(dave, chatId, message) {
    const userMessage = message.message.conversation || message.message.extendedTextMessage?.text || '';
    const text = userMessage.split(' ').slice(1).join(' ');

    if (!text) {
        await dave.sendMessage(chatId, { text: 'Please provide text after the .attp command.' });
        return;
    }
    ...
    try {
        ...
        await dave.sendMessage(chatId, { text: 'Failed to generate the sticker. Please try again later.' });
    }
}
