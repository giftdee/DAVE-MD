...
async function sendAnimu(dave, chatId, message, type) {
...
async function animeCommand(dave, chatId, message, args) {
    ...
    try {
        if (!sub) {
            ...
            try {
                ...
                await dave.sendMessage(chatId, { text: `Usage: .animu <type>\nTypes: ${apiTypes}` }, { quoted: message });
            } catch {
        ...
        if (!supported.includes(sub)) {
            await dave.sendMessage(chatId, { text: `❌ Unsupported type: ${sub}. Try one of: ${supported.join(', ')}` }, { quoted: message });
            return;
        }

        await sendAnimu(dave, chatId, message, sub);
    } catch (err) {
        console.error('Error in animu command:', err);
        await dave.sendMessage(chatId, { text: '❌ An error occurred while fetching animu.' }, { quoted: message });
    }
}
