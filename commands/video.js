...
async function videoCommand(dave, chatId, message) {
    try {
        ...
        if (!searchQuery) {
            await dave.sendMessage(chatId, { text: 'What video do you want to download?' }, { quoted: message });
            return;
        }
        ...
        if (searchQuery.startsWith('http://') || searchQuery.startsWith('https://')) {
            ...
            if (!videos || videos.length === 0) {
                await dave.sendMessage(chatId, { text: 'No videos found!' }, { quoted: message });
                return;
            }
        ...
        if (!urls) {
            await dave.sendMessage(chatId, { text: 'This is not a valid YouTube link!' }, { quoted: message });
            return;
        }
        ...
        try {
            ...
            if (meta?.success && meta?.result?.download_url) {
                ...
                await dave.sendMessage(chatId, { text: 'Failed to fetch video from the API.' }, { quoted: message });
                return;
            }
        } catch (e) {
        ...
        try {
            ...
            if (err.response && err.response.status === 403) {
                ...
                await dave.sendMessage(chatId, { text: 'Failed to download the video file.' }, { quoted: message });
                return;
            }
        }
        ...
        if (download403) {
            ...
            try {
                ...
                await dave.sendMessage(chatId, { text: 'Failed to download the video file from alternate CDN.' }, { quoted: message });
                return;
            }
        }
        if (!buffer || buffer.length < 1024) {
            await dave.sendMessage(chatId, { text: 'Downloaded file is empty or too small.' }, { quoted: message });
            return;
        }
        ...
        try {
            ...
            if (!fs.existsSync(convertedFile)) {
                await dave.sendMessage(chatId, { text: 'Converted file missing.' }, { quoted: message });
                return;
            }
            const stats = fs.statSync(convertedFile);
            const maxSize = 62 * 1024 * 1024; // 62MB
            if (stats.size > maxSize) {
                await dave.sendMessage(chatId, { text: 'Video is too large to send on WhatsApp.' }, { quoted: message });
                return;
            }
            ...
            try {
                if (!fs.existsSync(tempFile)) {
                    await dave.sendMessage(chatId, { text: 'Temp file missing.' }, { quoted: message });
                    return;
                }
        ...
        await dave.sendMessage(chatId, { text: 'Download failed: ' + (error?.message || 'Unknown error') }, { quoted: message });
    }
}
