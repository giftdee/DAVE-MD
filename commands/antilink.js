...
async function handleAntilinkCommand(dave, chatId, userMessage, senderId, isSenderAdmin) {
    try {
        if (!isSenderAdmin) {
        ...
        if (!action) {
            ...
            await dave.sendMessage(chatId, { text: usage });
            return;
        }
        ...
        switch (action) {
            case 'on':
                ...
                if (existingConfig?.enabled) {
                    await dave.sendMessage(chatId, { text: '*_Antilink is already on_*' });
                    return;
                }
            ...
            case 'off':
                ...
                await dave.sendMessage(chatId, { text: '*_Antilink has been turned OFF_*' });
                break;
            ...
            default:
                await dave.sendMessage(chatId, { text: `*_Use ${prefix}antilink for usage._*` });
        }
    } catch (error) {
        console.error('Error in antilink command:', error);
        await dave.sendMessage(chatId, { text: '*_Error processing antilink command_*' });
    }
}

async function handleLinkDetection(dave, chatId, message, userMessage, senderId) {
    ...
    if (shouldDelete) {
        ...
        await dave.sendMessage(chatId, { text: `Warning! @${senderId.split('@')[0]}, posting links is not allowed.`, mentions: mentionedJidList });
    } else {
