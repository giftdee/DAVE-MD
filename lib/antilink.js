 ...
 * @param {object} dave - The socket object to use for sending messages.
 */
async function Antilink(msg, dave) {
	...
	try {
		...
		await dave.sendMessage(jid, { delete: msg.key });
		...
		switch (action) {
			...
			case 'kick':
				await dave.groupParticipantsUpdate(jid, [sender], 'remove');
				await dave.sendMessage(jid, {
