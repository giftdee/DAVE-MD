...
async function addCommandReaction(dave, message) {
    try {
        if (!isAutoReactionEnabled || !message?.key?.id) return;
        
        const emoji = getRandomEmoji();
        await dave.sendMessage(message.key.remoteJid, {
            react: {
                text: emoji,
                key: message.key
            }
...
async function handleAreactCommand(dave, chatId, message, isOwner) {
    try {
        if (!isOwner) {
