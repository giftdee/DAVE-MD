...
async function handleAntiBadwordCommand(dave, chatId, message, match) {
    if (!match) {
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return dave.sendMessage(chatId, { text: '*AntiBadword is already enabled for this group*' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return dave.sendMessage(chatId, { text: '*AntiBadword has been enabled. Use .antibadword set <action> to customize action*' });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return dave.sendMessage(chatId, { text: '*AntiBadword is already disabled for this group*' });
        }
        await removeAntiBadword(chatId);
        return dave.sendMessage(chatId, { text: '*AntiBadword has been disabled for this group*' });
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return dave.sendMessage(chatId, { text: '*Invalid action. Choose: delete, kick, or warn*' });
        }
        await setAntiBadword(chatId, 'on', action);
        return dave.sendMessage(chatId, { text: `*AntiBadword action set to: ${action}*` });
    }

    return dave.sendMessage(chatId, { text: '*Invalid command. Use .antibadword to see usage*' });
}

async function handleBadwordDetection(dave, chatId, message, userMessage, senderId) {
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    switch (antiBadwordConfig.action) {
        ...
        case 'kick':
            try {
                await dave.groupParticipantsUpdate(chatId, [senderId], 'remove');
                await dave.sendMessage(chatId, {
